    ...
    try {
        pool.getConnection(function (error, database) {
            if (error) {
                ...
                var sqlquery = "update permintaan set flag_selesai=2 where flag_selesai=0 and due_date<date(now())";
                database.query(sqlquery, (error, rows) => {
                    database.release();
                    if (error) {
                        console.log('Query error cron set overdue refuse', error);

                    } else {
                        database.commit(function (err) {
                            if (err) {
                                console.log('Error commit cron set overdue refuse', error);
    ...
    try {
        pool.getConnection(function (error, database) {
            if (error) {
                ...
                var sqlquery = "select a.*, b.nama from permintaan a, pengguna b where a.idpengguna=b.idpengguna and flag_selesai=0 and (DATEDIFF(due_date,now())=0 or DATEDIFF(due_date,now())=1);";
                    ...
                    if (error) {
                        ...
                        result.forEach(element => {
                            ...
                            let notificationMessage = {
                                notification: {
                                    title: `Hi ${element.nama}, ada permintaan yang akan expired.`,
                            ...
                            fcmadmin.messaging().sendToTopic("RMSPERMINTAAN", notificationMessage)
                                .then(function (response) {
                                    ...
                                    console.log('Notifikasi cron permintaan expired berhasil dikirim', response);
                                }).catch(function (error) {
                                    console.log('Notifikasi gagal dikirim : ', error);
                                })
                        });
                    }
                })
